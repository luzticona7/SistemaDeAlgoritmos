(function(){
  ...
  function buildAdjacencyMatrix(){
    ...
    const createMatrixTable = (target, isDark = true) => {
      ...
      thead.innerHTML = '<tr><th></th>' + ordered.map(o=>`<th>${o.label}</th>`).join('') + '</tr>';
      target.appendChild(thead);
      const tbody = document.createElement('tbody');
      for(let i=0;i<n;i++){
        const row = document.createElement('tr');
        row.innerHTML = `<th>${ordered[i].label}</th>` + M[i].map(v=>`<td>${Number.isInteger(v)?v:v.toFixed(2)}</td>`).join('');
  ...
  document.getElementById('btn-compute-critical').onclick = () => {
  ...
  // computeCriticalPath function (mantiene la misma lógica)
  function computeCriticalPath(mode = 'max') {
    if (nodes.length === 0 || edges.length === 0) {
      showWarning('Agrega nodos y aristas primero.');
      return;
    }
    ...
    for (let e of edges) {
      if (mode === 'max') {
        if (e.weight == null || e.weight <= 0) {
          showWarning('Todas las aristas deben tener duración positiva (> 0) para maximizar (CPM).');
          return;
        }
      } else {
        if (e.weight == null || e.weight < 0) {
          showWarning('Todas las aristas deben tener duración >= 0 para minimizar (ruta más corta).');
          return;
        }
      }
    }
    ...
    if (mode === 'max') {
      const negEdges = edges.map(e => ({
        u: nodeIndex.get(e.source),
        v: nodeIndex.get(e.target),
        w: -e.weight
      }));
      ...
      h[artificial] = 0;
      for (let i = 0; i < n; i++) {
        for (let e of allEdgesNeg) {
          if (h[e.u] !== Infinity && h[e.u] + e.w < h[e.v]) {
            h[e.v] = h[e.u] + e.w;
          }
        }
      }
      ...
      if (hasCycle) {
        showWarning('Error: El grafo tiene un ciclo (no es DAG). Para CPM la red debe ser acíclica.');
        return;
      }

      const reweightedNegEdges = negEdges.map(e => ({
        u: e.u,
        v: e.v,
        w: e.w + h[e.u] - h[e.v]
      }));
      ...
      for (let src = 0; src < n; src++) {
        ...
        for (let i = 0; i < n; i++) {
          ...
          for (let e of reweightedNegEdges.filter(e => e.u === u)) {
        ...
        for (let v = 0; v < n; v++) {
          if (dist[v] < Infinity) {
            distMatrix[src][v] = dist[v] - h[src] + h[v];
          }
        }
      }

      if (distMatrix[start][end] === Infinity) {
        showWarning(`No hay ruta desde ${orderedNodes[start].label} hasta ${orderedNodes[end].label}.`);
        return;
      }
      ...
      if (path.length >= 2) {
        ...
        for (let i = 0; i < pathIds.length - 1; i++) {
          ...
          edges.forEach(e => {
            ...
            else if ((e.type === 'undirected' || e.type === 'bidirectional') && ((e.source === uId && e.target === vId) || (e.source === vId && e.target === uId))) e.isCritical = true;
          });
        }
      }
      ...
      resultDiv.innerHTML = `Ruta crítica (Maximizar / Johnson): ${pathLabels}<br>Duración total: ${criticalDuration.toFixed(0)}<br><span style="color: pink;">¡Atención! Esta es la Ruta Crítica, resaltada en rojo ★</span>`;
    ...
    for (let e of edges){
      ...
      const w = (e.weight == null || isNaN(e.weight)) ? 1 : Number(e.weight);
      if(distStart[u] === Infinity || distStart[v] === Infinity) {
        e.slack = undefined;
      } else {
    ...
    path = path.reverse();
    if(path[0] !== start) {
      showWarning('No fue posible reconstruir la ruta más corta (error interno).');
      return;
    }
    ...
    const resultDiv = document.getElementById('critical-path-result');
    resultDiv.innerHTML = `Ruta mínima (Minimizar / Shortest): ${pathLabels}<br>Duración total: ${criticalDuration.toFixed(0)}<br><span style="color: lightgreen;">¡Atención! Esta es la Ruta Crítica, resaltada en rojo ★</span>`;
